...
Функция ПолучитьСправочникПоНаименованию(ИмяСправочника,Наименование,Владелец=Неопределено, ОсновнаяЕдиница=Неопределено, Родитель=Неопределено, ШифрРабот = Неопределено);
	...
	// {{ Начало BI Group: Ержанұлы Б. 07.08.2018 14:42
	// Основание: Заявка № 201 368
	Если ИмяСправочника = "ИНАГРО_ВидыРаботПроект" Тогда
		Если НЕ ШифрРабот = Неопределено Тогда
			Работы = ЭлементСсылка.ПолучитьОбъект();
			Если Не ЗначениеЗаполнено(Работы.ШифрПозицииНорматива) ИЛИ Работы.ШифрПозицииНорматива <> ШифрРабот Тогда
				Работы.ШифрПозицииНорматива = ШифрРабот;
				Работы.ОбменДанными.Загрузка = Истина;
				Работы.Записать();
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗагрузитьПроекты(СписокИдентификаторов=Неопределено) Экспорт
	...
	Пока ВыборкаПроекты.Следующий() Цикл
		...
		ИдентификаторыРабот = Новый Соответствие;
		 // ++  BI-Digital: p.virich 2835  2018-08-09
		 с_ОбьемыРабот = Новый Соответствие;
		 // --  BI-Digital: p.virich 2835  2018-08-09 
		...
		Пока ВыборкаРаботы.НайтиСледующий(СтруктураПоиска) Цикл
			...
			// {{ Начало BI Group: Ержанұлы Б. 07.08.2018 14:42
			// Основание: Заявка № 201 368
			Строка.ВидРаботы = ПолучитьСправочникПоНаименованию("ИНАГРО_ВидыРаботПроект",ВыборкаРаботы.Task_Name,ПроектСсылка, ЕдиницаИзмеренияСсылка, ,ВыборкаРаботы.task_normative_code);
			...
			// ++  BI-Digital: p.virich 2835  2018-08-09
			с_ОбьемыРабот.Вставить(Строка.КлючСтроки, Строка.Объем);
			// --  BI-Digital: p.virich 2835  2018-08-09 
		...
		Пока ВыборкаРесурсы.НайтиСледующий(СтруктураПоиска) Цикл
			...
			Строка.ВидРаботы = ИдентификаторыРабот.Получить(Строка.КлючСтроки);
			 ...
			 // ++  BI-Digital: p.virich 2835  2018-08-09
			 с_ОбъемРабот = с_ОбьемыРабот.Получить(Строка.КлючСтроки);
			 Если с_ОбъемРабот <> Неопределено и с_ОбъемРабот <> 0 Тогда
 				Строка.НормаРасхода = Строка.Количество/ с_ОбъемРабот;
 			 КонецЕсли;
 			 // --  BI-Digital: p.virich 2835  2018-08-09 
...
Процедура ВыгрузитьФакт() Экспорт
	...
	//{{ BI-Digital: Ержанұлы Б. по заявке 229322 от 02.10.2018 9:30
	ПараметрыБазыДанных = ПолучитьПараметрыБазыДанных();
	//}} BI-Digital: Ержанұлы Б. по заявке 229322 от 02.10.2018 9:30
