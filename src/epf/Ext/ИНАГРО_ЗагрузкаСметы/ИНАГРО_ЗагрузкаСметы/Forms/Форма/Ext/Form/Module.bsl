...
&НаСервереБезКонтекста
Функция НазначитьНовыйКлюч()
  	
	//КлючСтроки = Новый УникальныйИдентификатор();	
	
	КлючСтроки = РегистрыСведений.ИНАГРО_УГПРРаботы.НовыйКлючРаботы();
...
Процедура ЗаполнитьПоЛокальнойСмете(ОбъектСмета)
	...
	Пока  Выборка.Следующий() Цикл
 		...
 		Если Выборка.Расчет Тогда
			
			Позиция =  Выборка.Ссылка;
			Родитель = Выборка.ИДРодителя;
			...
			Если ТипЗнч(Позиция)=Тип("ДокументСсылка.СМ_ПозицияЛокальнойСметы") Тогда
				...
				НС.ВидРасценки = ВидРасценки;
				НС.ЕдиницаИзмерения = ИНАГРО_ОбщегоНазначения.ПолучитьЗначениеРеквизита(Позиция, "ЕдиницаИзмерения");
				НС.ШифрРасценки = ИНАГРО_ОбщегоНазначения.ПолучитьЗначениеРеквизита(Позиция, "ШифрРасценки");
	...
	//Объект.ПозицииСметы.Сортировать("НПП");
...
Функция ЗаполнитьПоСметеНаСервере()
	...
	ИначеЕсли   Тип("ДокументСсылка.СМ_СводнаяСмета") = ТипЗнч(Объект.Смета)  Тогда
		...
		//  #1164 -   случае, если пользователь загружает Сводную смету, структуру WBS УГПРа начинать не с верхнего уровня (Проекта), а со следующего
		 //Ситис Вирич П.В. 2085  Начало 2018-03-30
		// передумали - опять нужен верхний уровень 
 		НС = Объект.ПозицииСметы.Добавить();
		НС.КлючСвязи = 	"00000000-0000-0000-0000-000000000000";
		НС.КлючСтроки = Объект.Смета.УникальныйИдентификатор();
		НС.Ссылка = Объект.Смета;
		...
		НС.ЭтоРабота = Ложь;
		НС.ЭтоМатериал = Ложь;
		//Ситис Вирич П.В. 2085  Конец 2018-03-30 
		...
		Для каждого СтрОбъектСмета Из ТЧСметы Цикл
			...
			Если СтрОбъектСмета.Расчет Тогда
				...
				ИначеЕсли   Тип("ДокументСсылка.СМ_ОбъектнаяСмета") = ТипЗнч(ОбъектСмета)  Тогда
					...
					НС = Объект.ПозицииСметы.Добавить();
					//НС.КлючСвязи = 	 Объект.Смета.УникальныйИдентификатор(); // сводная смета  #1164 - смвыше
					 //Ситис Вирич П.В. 2085  Начало 2018-03-30
					 НС.КлючСвязи = 	 Объект.Смета.УникальныйИдентификатор();
 					//НС.КлючСвязи ="00000000-0000-0000-0000-000000000000";
...
Процедура ПозицииСметы_в_ДЗКлиент()
	...
	НайденнаяСтрока = ПолучитьВеткуПоКлючуСтроки(РаботыДЗ, ТекКлючСтроки, РаботыДЗ, "ДЗ");
...
Процедура ПозицииСметы_в_ДЗ()
	...
	тПозицииСметы = Объект.ПозицииСметы.Выгрузить();
	тПозицииСметы.Индексы.Добавить("КлючСвязи");
	
	ПреобразоватьВДЗРекурсия(тДерево, тПозицииСметы, КореньДерева);
...
Процедура СтруктураУГПР_в_ДЗСервер()
	...
	тСтруктураУГПР = Объект.СтруктураУГПР.Выгрузить();
	тСтруктураУГПР.Индексы.Добавить("КлючСвязи");

	ПреобразоватьВДЗРекурсия(тДерево, тСтруктураУГПР,  "00000000-0000-0000-0000-000000000000");
...
Процедура СтруктураУГПР_в_ДЗ()
	...
	НайденнаяСтрока = ПолучитьВеткуПоКлючуСтроки(РаботыДЗ2, ТекКлючСтроки, РаботыДЗ2, "ДЗ2");
...
Процедура РаботыДЗ2_В_СтруктураУГПР_Сервер()
 	Объект.СтруктураУГПР.Очистить();
	ВыгрузитьДеревоЗначенийВТаблицуЗначений(РеквизитФормыВЗначение("РаботыДЗ2"), Объект.СтруктураУГПР);
КонецПроцедуры

&НаСервере
Процедура РаботыДЗ_В_ПозицииСметы_Сервер()
 	Объект.ПозицииСметы.Очистить();
	ВыгрузитьДеревоЗначенийВТаблицуЗначений(РеквизитФормыВЗначение("РаботыДЗ"), Объект.ПозицииСметы);
КонецПроцедуры

 &НаКлиенте
Процедура Тест(Команда)
	
	Если Объект.Смета = Неопределено или Объект.Смета.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьЭлементУГПР(Команда)
	...
	Если ВыделенныеСтрокиКоличество > 1 Тогда
		...
		Если УказыватьКлассификаторы Тогда
		...
		иначе //Если УказыватьКлассификаторы Тогда

			Для каждого ТекСтрока Из  ВыделенныеСтроки Цикл
				ДобавитьЭлементУГПРПоСтроке(РаботыДЗ.НайтиПоИдентификатору(ТекСтрока));
			КонецЦикла;

		КонецЕсли; // УказыватьКлассификаторы
...
Процедура ОбработатьКонструктив_Оповещение(Результат, Параметры) Экспорт
	...
	Если ТекущиеДанныеСмета <> Неопределено Тогда
		...
		НС.КлючСтроки = НазначитьНовыйКлюч();
...
Процедура ПозицииСметы_Отобрано(КлючСтроки, Отобрано)
	...
	Ветка = ПолучитьВеткуПоКлючуСтроки(РаботыДЗ, КлючСтроки, РаботыДЗ, "ДЗ");
...
Процедура УстановитьДоступностьРеквизитов(Элемент, ТекущиеДанные, ТекущееПоле)
	...
	СтруктураТолькоЧтение.Вставить(Элемент.Имя + "НПП", Истина);
...
Процедура ДобавитьВТекущуюПозицию_Оповещение(Идентификатор)
	...
	Если ТекущиеДанныеСмета<> Неопределено и ТекущиеДанные <> Неопределено Тогда
		...
		Если ТекущиеДанные.ЭтоРабота  Тогда
			...
			НС2.КлючСвязи = ТекущиеДанные.КлючСвязи;
...
&НаСервереБезКонтекста
Функция СоздатьВерсиюДляЗагрузки(Наименование, Организация, Проект)

	НоваяВерсия = Справочники.ИНАГРО_Версия.СоздатьЭлемент();
	НоваяВерсия.Владелец =  Проект;
	НоваяВерсия.Наименование = Наименование;
	НоваяВерсия.Организация = Организация;
	НоваяВерсия.ВидВерсии = Перечисления.ИНАГРО_ВидВерсии.Прочая;
...
Процедура СоздатьУГПР(ДанныеФормы, ДополнительныеПараметры = Неопределено)
	...
	Если ДополнительныеПараметры <> Неопределено Тогда
		...
		НаименованиеВерсии = ""+НаименованиеВерсии + "+ Загрузка из 1с сметы от "+ТекущаяДатаСеанса();
 	иначе
	...
	Для каждого ТекСтрока  Из ТаблицаСоответствий Цикл
		...
		Если ТекСтрока.ЭтоРабота или ТекСтрока.ЭтоМатериал Тогда
		...
		таблРесурсов = СМ_СметаРасчетСервер.ПолучитьТаблицуСтоимостиРесурсовСметыОптимизация(ПозицииСписок);

		Для каждого ТекСтрРес  Из таблРесурсов Цикл
			
			ИмяТЧ = ИмяТЧПоВидуРесурса(ТекСтрРес.ВидРесурса);
			
			ТаблицаРесурсов = НовыйДок[ИмяТЧ];
			...
			// Ресурсы
			Коэф = 1;
			Если ТекСтрока.РаспределениеСметнойСтоимости <>  0 Тогда
				Коэф = ТекСтрока.РаспределениеСметнойСтоимости;
			КонецЕсли;

			НС.Количество = Окр(ТекСтрРес.РасходПоНорме * Коэф, 3);
			НС.Цена = ТекСтрРес.ЦенаТекущая;
			НС.Сумма = НС.Количество* НС.Цена; // стоимость в результатах запроса какаая то вообще левая ... поэтому множим
			
			Если ИмяТЧ = "РесурсыМашинные" Тогда
				НС.СуммаЗПМашинистов = Окр(ТекСтрРес.СтоимостьТекущаяЗМ*Коэф, 3);
			КонецЕсли;
		...
		Если ТипЗнч(Позиция)=Тип("ДокументСсылка.СМ_ПозицияЛокальнойСметы") Тогда
			...
			Для каждого СтрокаСостава Из Позиция.СоставРабот Цикл
				...
				Операция = Справочники.ИНАГРО_Операции.СоздатьНайтиЭлемент(Лев(СтрокаСостава.НаименованиеРаботы, 150));
	...
	НовыйДок.СметнаяПрибыль = НовыйДок.СметнаяПрибыль + СметнаяПрибыль;
	...
	НовыйДок.НакладныеРасходыРаспред = НовыйДок.НакладныеРасходыРаспред + НР;
...
Процедура СметаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтруктураПараметров.Вставить("Стройка", ОбьектСтроительстваПоПроектуСервер(Объект.Проект) );
...
&НаСервере
Функция ОбьектСтроительстваПоПроектуСервер(Проект)
	 Возврат ИНАГРО_УСПОбщиеФункцииКлиентСервер.ОбьектСтроительстваПоПроектуСервер(Проект);
КонецФункции
...
Процедура ЗагрузитьСуществующуюСтруктуруУГПР()
	...
	|	ИНАГРО_СоответствияСметаУГПР.НПП КАК НПП,
	|	ИНАГРО_СоответствияСметаУГПР.ШифрРасценки
	|ИЗ
	...
	Если не РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий()  Цикл
			...
			Если ЕстьВхождениеВТекущуюСмету(Выборка.КлючСтрокиСмета, Объект.ПозицииСметы, ХэшКлючей) Тогда 
				...
				Если Выборка.ПозицияЛокальнойСметы = Объект.Смета Тогда  // дерево начинаем с "нуля"
					НС.КлючСвязи = 	"00000000-0000-0000-0000-000000000000";
				КонецЕсли;
				
				// експеримент
				Если НС.КлючСвязи = "00000000-0000-0000-0000-000000000000" и Выборка.ПозицияЛокальнойСметы <> Объект.Смета Тогда
					//TODO Надо задать ключ связи	
				КонецЕсли;
...
Функция ЕстьВхождениеВТекущуюСмету(Ключ, ТЧПозицииСметы, ХэшКлючей)
	...
	// НайденноеЗначение =   ХэшКлючей.Найти("_"+Ключ, НайденноеЗначение);
	//Если Тогда
...
Процедура ТаблицаСоответствийПередУдалением(Элемент, Отказ)
	...
	Если ТекущиеДанные <> Неопределено  Тогда
		...
		ПараметрыОтбора = Новый Структура("КлючСтрокиСмета", ТекущиеДанные.КлючСтрокиСмета);
		СтрокиКУдалению = Объект.ТаблицаСоответствий.НайтиСтроки(ПараметрыОтбора);
		
		Для каждого СтрокаУдаления  Из СтрокиКУдалению Цикл
			
			СтруктураОтбора.Вставить("ПозицияЛокальнойСметы",  СтрокаУдаления.ПозицияЛокальнойСметы);
			СтруктураОтбора.Вставить("Основная",  СтрокаУдаления.Основная);
			СтруктураОтбора.Вставить("КлючСтроки",  СтрокаУдаления.КлючСтроки);
			
			УдалитьЗаписьРегистраСоответствий(СтруктураОтбора);
			
			Объект.ТаблицаСоответствий.Удалить(СтрокаУдаления);
...
Процедура РаспределитьПоРаботам(Команда)
	...
	Если ТекущиеДанные <> Неопределено и (ТекущиеДанные.ЭтоРабота или ТекущиеДанные.ЭтоМатериал) Тогда
		...
		СтруктураПараметров.Вставить("ОбъемКРаспределению", ТекущиеДанные.ОбъемРабот);
...
Процедура ПриЗакрытииФормыРаспределения(Результат, ТекПараметр) Экспорт
	...
	ИтогСумма = 0;
	// Рассчитаем общий итог для составления пропорции
	Для каждого КлючСтроки  Из Результат Цикл
		ИтогСумма = ИтогСумма + КлючСтроки.Значение;
	КонецЦикла;
	...
	ИтогРаспределенияДляКонтроля = 0;
	// запишем пропорции
	Для каждого КлючСтроки  Из Результат Цикл
		...
		ТекущиеДанные =  ПолучитьВеткуПоКлючуСтроки(РаботыДЗ2, КлючСтроки.Ключ, РаботыДЗ2, "ДЗ2");
		...
		Если ТекущиеДанныеСмета <> Неопределено и ТекущиеДанные <> Неопределено Тогда
			...
			НС2.Основная = Ложь;
			НС2.КлючСтрокиСмета = ТекущиеДанныеСмета.КлючСтроки;
 			ЗначениеКоэф = Окр(КлючСтроки.Значение / ИтогСумма,3);
 			НС2.РаспределениеСметнойСтоимости = ЗначениеКоэф;
			
			ИтогРаспределенияДляКонтроля = ИтогРаспределенияДляКонтроля + ЗначениеКоэф;
	...
	Если ИтогРаспределенияДляКонтроля <> 1 Тогда
		НС2.РаспределениеСметнойСтоимости = НС2.РаспределениеСметнойСтоимости + (1-ИтогРаспределенияДляКонтроля);
	КонецЕсли;
...
&НаКлиенте
Функция ПолучитьВеткуПоКлючуСтроки(Дерево, КлючСтроки, ПолноеДерево, ВидКэша="ДЗ") 
	...
	Если  КэшКлючейДерева = Неопределено Тогда
		КэшКлючейДерева = новый	 Соответствие;
	КонецЕсли;
	
	НайденноеЗначение = КэшКлючейДерева.Получить(ВидКэша+КлючСтроки);
	
	Если НайденноеЗначение <> Неопределено Тогда
		
		НайденнаяСтрока = ПолноеДерево.НайтиПоИдентификатору(НайденноеЗначение);
		
		Если НайденнаяСтрока <> Неопределено
			и  НайденнаяСтрока.КлючСтроки = КлючСтроки Тогда //доп контроль
	...
	// в кеше нет или некорректные данные
	
	ОбновитьКеш();

	// еще раз ищем
	
	НайденнаяСтрока = ПолучитьВеткуПоКлючуСтроки(Дерево, КлючСтроки, ПолноеДерево, ВидКэша);
	...
	//// оставим для теста - старый поиск перебором
	//
	//
	//Для Каждого Ветка Из Дерево.ПолучитьЭлементы() Цикл
	//	
	...
	//	Иначе
	//		
	//		НайденнаяСтрока = ПолучитьВеткуПоКлючуСтроки(Ветка, КлючСтроки, ПолноеДерево, ВидКэша);
	//		
	//		Если НайденнаяСтрока <> Неопределено Тогда
	//			
	//			КэшКлючейДерева.Вставить(ВидКэша+КлючСтроки, НайденнаяСтрока.ПолучитьИдентификатор());
	//			
	//			Возврат НайденнаяСтрока;
	//			
	//		КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьКеш()
	
	Если КэшКлючейДерева = Неопределено Тогда
		КэшКлючейДерева = новый Соответствие;
	КонецЕсли;
	
	КэшКлючейДерева.Очистить();
	
	СоздатьПолныйКешРекурсия(РаботыДЗ, "ДЗ");
	СоздатьПолныйКешРекурсия(РаботыДЗ2, "ДЗ2");
	
КонецПроцедуры

&НаКлиенте
Процедура СоздатьПолныйКешРекурсия(Дерево, ВидКэша)
	
	Для Каждого Ветка Из Дерево.ПолучитьЭлементы() Цикл
		
		КэшКлючейДерева.Вставить(ВидКэша+Ветка.КлючСтроки, Ветка.ПолучитьИдентификатор());
		
		СоздатьПолныйКешРекурсия(Ветка, ВидКэша);
...
Процедура СоздатьАналогичнуюСтруктуру(Команда)
	...
	РаботыДЗ_В_ПозицииСметы_Сервер();// перезапишем чтобы воссоздать сортировку дерева а не оригинальную сметы (часто кривая)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("СоздатьУГПРСтруктуру", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, "Текущая структура УГПР будет утеряна, продолжить?",РежимДиалогаВопрос.ДаНет, 60);

КонецПроцедуры

 &НаКлиенте
Процедура СоздатьУГПРСтруктуру(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Для каждого ТекущиеДанныеСмета Из Объект.ПозицииСметы Цикл
			...
			ИначеЕсли  ТекущиеДанныеСмета.ЭтоМатериал  Тогда
				...
				// будет отдельная ветка ниже
				
				Если не АвтоРазмещениеМатериалов Тогда
					Продолжить;
				КонецЕсли;
								
			иначе //Если Это не Работа и не материал 
			...
			КонецЕсли; //Если ТекущиеДанныеСмета.ЭтоРабота  Тогда
			
			// Для материалов отдельная ветка - их надо поместить только в позицию			
			Если ТекущиеДанныеСмета.ЭтоРабота или не ТекущиеДанныеСмета.ЭтоМатериал   Тогда
				...
				// TODO ключи переназначить обязательно!!! но надо сохранить структуру - назначить новые ключи связи тоже
				// тоесть это делать после обработки всех связей
				...
				НС2.КлючСтрокиСмета = ТекущиеДанныеСмета.КлючСтроки;
 				...
 				Если ТекущиеДанныеСмета.ЭтоРабота Тогда
					ТекущаяПозиция = НС;
				иначе
					ТекущаяПозиция = Неопределено;
				КонецЕсли; //
				
			иначе // Ветка материалов
				
				//помещать материалы в работу которая выше в дереве в этой же ветке, если есть такая 
				
				Если ТекущаяПозиция <> Неопределено  и АвтоРазмещениеМатериалов Тогда
					
					НС2 = Объект.ТаблицаСоответствий.Добавить();
					
 					ЗаполнитьЗначенияСвойств(НС2, ТекущиеДанныеСмета);
					
					НС2.КлючСтроки = ТекущаяПозиция.КлючСтроки;
					НС2.ВидРаботы =  ТекущаяПозиция.ВидРаботы;
					НС2.ПозицияЛокальнойСметы = ТекущиеДанныеСмета.Ссылка;
					НС2.Основная = Ложь;
					НС2.КлючСтрокиСмета = ТекущиеДанныеСмета.КлючСтроки;
				...
				КонецЕсли; // Если ТекущаяПозиция <> Неопределено Тогда
				
 			КонецЕсли;
			
		КонецЦикла; //  каждого ТекущиеДанныеСмета Из Объект.ПозицииСметы Цикл

		СтруктураУГПР_в_ДЗСервер();

	КонецЕсли; // 	 РезультатВопроса = КодВозвратаДиалога.Да Тогда
...
Процедура НазначитьОсновнойРаботой(Команда)
	...
	Если  ТекущиеДанные <> Неопределено Тогда
		...
		Если не ТекущиеДанные.ЭтоРабота или ТекущиеДанные.Основная Тогда
			Возврат;
		КонецЕсли;

		Ветка = ПолучитьВеткуПоКлючуСтроки(РаботыДЗ2, ТекущиеДанные.КлючСтроки, РаботыДЗ2, "ДЗ2");
		...
		Если Ветка <> Неопределено Тогда
			...
			Если ЭлементWBS = Ветка.ВидРаботы Тогда // она и так основная
				Возврат;
			КонецЕсли;
...
&НаКлиенте
Процедура ТаблицаСоответствийПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)

	ИДПриемник = Строка;
	
	// Если ИДПриемник = Неопределено => Корень дерева
	Приемник = ?(ИДПриемник = Неопределено, Неопределено,
	Объект.ТаблицаСоответствий.НайтиПоИдентификатору(ИДПриемник));
	...
	Для Каждого ИДИсточник Из МассивИДИсточник Цикл
		...
		иначе	
			...
			Идентификатор =  ИДИсточник.ПолучитьИдентификатор();
			
			ДобавитьВТекущуюПозицию_Оповещение(Идентификатор);
...
&НаКлиенте
Процедура ДобавитьвУГПР(Команда)
...
Процедура ОбработкаВыбораДокумента(РезультатВыбора, ДополнительныеПараметры) Экспорт 
	...
	ПоказатьВопрос(ОписаниеОповещения, "Будет создана версия УГПР с добавлением работ сметы, продолжить?", РежимДиалогаВопрос.ДаНет, 60);
...
&НаКлиенте
 Процедура РаботыДЗ2ПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	 Отказ = Копирование;
 КонецПроцедуры

 &НаКлиенте
 Процедура ПриОткрытии(Отказ)
	 АвтоРазмещениеМатериалов = истина;
 КонецПроцедуры
