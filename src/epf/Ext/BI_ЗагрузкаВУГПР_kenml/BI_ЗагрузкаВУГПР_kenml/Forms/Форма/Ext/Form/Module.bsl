...
Процедура РазобратьСписокХДТО(ТекущийЭлементДерева, РазделыХДТО)
	...
	Для каждого ЭлементСпискаХДТО Из РазделыХДТО Цикл  	
		...
		Если ЭтоРабота Тогда
			...
			Если не ЭтоРаботаРесурс и не ОтключитьАвтокоректированиеРесурсов Тогда
				
				Если не ЭтоРаботаБезРесурсов Тогда
...
Функция  ДобавитьЭлементВДерево(ТекущийЭлементДерева, ЭлементСпискаХДТО, ЭтоРабота = Ложь, ЭтоРесурс = Ложь, ЭтоРаботаРесурс = Ложь)
	...
	Если НовСтр.ЭтоРабота Тогда   // работа
		...
		ЗаполнитьСтоимости(ЭлементСпискаХДТО, НовСтр, ПоказательСП, флФорматСметы, РасчетНР1ОтРазницы, НеОкруглять);
...
Функция ДобавитьЭлементВДерево_Ресурс(ТекущийЭлементДерева, ЭлементСпискаХДТО, НовСтр)
	...
	Если ЭлементСпискаХДТО.Свойства().Получить("Цена") <> Неопределено Тогда
		...
		Если  флФорматСметы = 2018 или НеОкруглять Тогда // формат 2018 не округляем
...
Функция ЗаполнитьСтоимости(ЭлементСпискаХДТО, НовСтр, ПоказательСП, флФорматСметы, РасчетНР1ОтРазницы, НеОкруглять)
	...
	Если ЭлементСпискаХДТО.Свойства().Получить("СТОИМОСТЬ") <> Неопределено Тогда
		...
		КонецЕсли;
		Если НовСтр.СуммаМТ = 0 или СвойстваСтоимости.Свойства().Получить("МТ") = Неопределено Тогда
		...
		Если  СвойстваСтоимости.Свойства().Получить("ЗМ") <> Неопределено  Тогда
			Если НеОкруглять Тогда
				НовСтр.СуммаЗПМашинистов =  Окр(СтрокаВЧисло(СвойстваСтоимости.ЗМ), 2);
			иначе				
