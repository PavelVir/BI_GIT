...
Процедура РазобратьСписокХДТО(ТекущийЭлементДерева, РазделыХДТО)
	...
	Для каждого ЭлементСпискаХДТО Из РазделыХДТО Цикл  		
		...
		Если ЭлементСпискаХДТО.Свойства().Получить("РЕСУРС") <> Неопределено и не БезРесурсов Тогда 
			...
			Если флФорматСметы=1 или флФорматСметы=2 Тогда
				СкорректироватьОплатуМашинистов(НЭ);
			КонецЕсли;
			
			Если флФорматСметы=2 Тогда
...
Функция  ДобавитьЭлементВДерево(ТекущийЭлементДерева, ЭлементСпискаХДТО, ЭтоРабота = Ложь, ЭтоРесурс = Ложь)
	...
	Если ЭтоРабота
		и (ЭлементСпискаХДТО.Тип = "6" или  ЭлементСпискаХДТО.Тип = "1" или  ЭлементСпискаХДТО.Тип = "2")  // Работа-ресурс 
...
Функция ЗаполнитьСтоимости(ЭлементСпискаХДТО, НовСтр, ПоказательСП, флФорматСметы)
	...
	Если ЭлементСпискаХДТО.Свойства().Получить("СТОИМОСТЬ") <> Неопределено Тогда
		...
		иначеЕсли СвойстваСтоимости.Свойства().Получить("ОТП") <> Неопределено  Тогда  //оборудование нет прямых затрат
			...
			//Часто неполные суммы, попробуем брать из цены
			
			//	НовСтр.Сумма =  СтрокаВЧисло(СвойстваСтоимости.ОТП);
			//	
			//	
			//	//если это оборудование, иногда стоимость подроблена по видам, проверим
			//	
			//	Если  СвойстваСтоимости.Свойства().Получить("ЗЧ") <> Неопределено Тогда
			//		НовСтр.Сумма = НовСтр.Сумма + СтрокаВЧисло(СвойстваСтоимости.ЗЧ);	
			//	КонецЕсли;
			//	
			//	Если  СвойстваСтоимости.Свойства().Получить("ТАРА") <> Неопределено Тогда
			//		НовСтр.Сумма = НовСтр.Сумма + СтрокаВЧисло(СвойстваСтоимости.ТАРА);	
			//	КонецЕсли;
			//	
			//	Если  СвойстваСтоимости.Свойства().Получить("ТР") <> Неопределено Тогда
			//		НовСтр.Сумма = НовСтр.Сумма + СтрокаВЧисло(СвойстваСтоимости.ТР);	
			//	КонецЕсли;
			//	
			//	Если  СвойстваСтоимости.Свойства().Получить("ЗГС") <> Неопределено Тогда
			//		НовСтр.Сумма = НовСтр.Сумма + СтрокаВЧисло(СвойстваСтоимости.ЗГС);	
			//	КонецЕсли;
			//	
			//	Если  СвойстваСтоимости.Свойства().Получить("КОМПЛ") <> Неопределено Тогда
			//		НовСтр.Сумма = НовСтр.Сумма + СтрокаВЧисло(СвойстваСтоимости.КОМПЛ);	
			//	КонецЕсли; 		
		...
		Иначе
			...
			Если ПоказательСП <> 0 Тогда 				
				...
				Если флФорматСметы <> 2 
					или (флФорматСметы = 2 и НовСтр.Тип <> "2") //на материалы не надо? утончить
					Тогда
...
Функция фАвтопомещениеРесурсовВРаботы(РаботыРаздела, ЭлементСпискаХДТО)
	...
	Если ЭлементСпискаХДТО.Тип = "6" Тогда
		НовСтр.Тип = "3";
	ИначеЕсли ЭлементСпискаХДТО.Тип = "1" Тогда
		НовСтр.Тип = "2";
	ИначеЕсли ЭлементСпискаХДТО.Тип = "2" Тогда 
		НовСтр.Тип = "3";
	Иначе
		НовСтр.Тип = "2";
	КонецЕсли;
