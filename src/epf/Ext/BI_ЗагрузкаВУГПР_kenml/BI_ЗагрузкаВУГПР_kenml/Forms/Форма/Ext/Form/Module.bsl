...
Функция ЗаполнитьСтоимости(ЭлементСпискаХДТО, НовСтр, ПоказательСП, флФорматСметы)
	...
	Если ЭлементСпискаХДТО.Свойства().Получить("СТОИМОСТЬ") <> Неопределено Тогда
		...
		// делаем в конце - чтобы были посчитаны НР для формата 2
...
Процедура СоздатьУГПР(ДанныеФормы, ДополнительныеПараметры = Неопределено)
	...
	Если ИтогСтоимостиПоРесурсам <> ИтогоПоСмете Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("По данным сметы "+ИтогоПоСмете+ ", по данным УГПР: "+ИтогСтоимостиПоРесурсам+" (Общая разница = "+ (ИтогоПоСмете-ИтогСтоимостиПоРесурсам)+")");
 	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АвтопомещениеРесурсовВРаботы = Истина;
...
// Добавлена опция размещения ресурсов в предыдущую работу сметы  (по умолчанию включена)