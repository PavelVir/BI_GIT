...
Функция ЗаполнитьСтоимости(ЭлементСпискаХДТО, НовСтр, ПоказательСП, флФорматСметы)
	...
	Если ЭлементСпискаХДТО.Свойства().Получить("СТОИМОСТЬ") <> Неопределено Тогда
		...
		// Материалы
		Если  СвойстваСтоимости.Свойства().Получить("МТ") <> Неопределено  Тогда
			НовСтр.СуммаМТ =  СтрокаВЧисло(СвойстваСтоимости.МТ);
		КонецЕсли;
		Если НовСтр.СуммаЕМ = 0 или СвойстваСтоимости.Свойства().Получить("МТ") = Неопределено Тогда
			Если  СвойстваСтоимостиЕд.Свойства().Получить("МТ") <> Неопределено  Тогда
				НовСтр.СуммаМТ =  СтрокаВЧисло(СвойстваСтоимостиЕд.МТ) * НовСтр.Объем;
			КонецЕсли;
		КонецЕсли;
		...
		НовСтр.СуммаВСЕГО =  СтрокаВЧисло(СвойстваСтоимости.ВСЕГО);
...
Функция СкорректироватьОсновнуюОплату(Ветка_Работа)
	...
	Если Ветка_Работа.СуммаЗП <> 0 или ЭлементОсновнаяЗП <> Неопределено Тогда

		Если (Ветка_Работа.СуммаЗП <> 0 
			и ЭлементОсновнаяЗП <> Неопределено  
			и ЭлементОсновнаяЗП.Сумма <>  Ветка_Работа.СуммаЗП) // не совпадает зп по работе в целом и по ресурсу
			
			или
			
			(Ветка_Работа.СуммаЗП =0 и ЭлементОсновнаяЗП <> Неопределено) // по работе ноль а ресурс с суммой есть (да бывает и такое)
		...
		Если тчРесурсы.Количество()  Тогда
			...
			РазницаКРаспределению =  Ветка_Работа.СуммаМТ - ИтогСуммаСуммаМТ;
 			...
 			Если ЭлементПрочиеМатериалы <> Неопределено и ЭлементПрочиеМатериалы.Сумма = 0 Тогда // если есть прочие материалы без стоимости то разницу вешаем только на них
				...
				ИтогСуммаСуммаМТ =  1;
				ЭлементПрочиеМатериалы.Сумма = 1;
			...
			Если ИтогСуммаСуммаМТ <> 0 и  ИтогСуммаСуммаМТ <> Ветка_Работа.СуммаМТ Тогда				
 				...
 				Для каждого ЭлементДерева Из МассивРесурсов Цикл
					...
					Разница =  Окр((РазницаКРаспределению / ИтогСуммаСуммаМТ) * ЭлементДерева.Сумма, 0);	
...
Функция СкорректироватьНакладныеРасходы(Ветка_Работа)	
	...
	ИначеЕсли Ветка_Работа.СуммаВСЕГО <> 0 и ПоказательСП = 0 Тогда // накладные расходы можно рассчитать, есть кривые сметы без НР и СП, сначало накинем НР
		...
		Разница = Ветка_Работа.СуммаВСЕГО - ОбщаяРесурснаяСебестоимость - Ветка_Работа.НакладныеРасходы;
		...
		иначе
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(" ОТРИЦАТЕЛЬНАЯ Разница в работе "+Ветка_Работа.КодWBS+"/"+Ветка_Работа.Название + " ПРОПУСКАЮ  - нужна проверка данных"+ Разница);
	...
	Если Ресурс <> Неопределено Тогда
		...
		Если  СтруктураПараметров.Тип = "1" Тогда  // РАБОЧИЕ (и машинисты)
			...
			Если СтруктураПроверяемыхРеквизитов.ИНАГРО_ТипРесурсов <> Перечисления.ИНАГРО_ТипыРесурсов.ТрудовыеРесурсы Тогда
				СтруктураПараметров.Вставить("ОписаниеОшибки", "Ожидали найти трудовой ресурс, а нашли "+СтруктураПроверяемыхРеквизитов.ИНАГРО_ТипРесурсов);
			Иначе
				Если СтруктураПроверяемыхРеквизитов.ВидРесурса <> Перечисления.СМ_ВидыРесурсов.ЗатратыТрудаРабочих 
					...
					СтруктураПараметров.Вставить("ОписаниеОшибки", "Ожидали найти трудовой ресурс, а нашли "+СтруктураПроверяемыхРеквизитов.ВидРесурса);
 				КонецЕсли;
			КонецЕсли;
		...
		ИначеЕсли  СтруктураПараметров.Тип = "2" Тогда  // МАТЕРИАЛЫ
			...
			Если СтруктураПроверяемыхРеквизитов.ИНАГРО_ТипРесурсов <> Перечисления.ИНАГРО_ТипыРесурсов.Материальные Тогда
				СтруктураПараметров.Вставить("ОписаниеОшибки", "Ожидали найти материальный ресурс, а нашли "+СтруктураПроверяемыхРеквизитов.ИНАГРО_ТипРесурсов);
			Иначе
				Если СтруктураПроверяемыхРеквизитов.ВидРесурса <> Перечисления.СМ_ВидыРесурсов.Материалы  Тогда	
					СтруктураПараметров.Вставить("ОписаниеОшибки", "Ожидали найти материальный ресурс, а нашли "+СтруктураПроверяемыхРеквизитов.ВидРесурса);
				КонецЕсли;
 			КонецЕсли;
		...
		ИначеЕсли  СтруктураПараметров.Тип = "3" Тогда  // МАШИНЫ И МЕХАНИЗМЫ		
			...
			Если СтруктураПроверяемыхРеквизитов.ИНАГРО_ТипРесурсов <> Перечисления.ИНАГРО_ТипыРесурсов.МашиныИМеханизмы Тогда
				СтруктураПараметров.Вставить("ОписаниеОшибки", "Ожидали найти машинный ресурс, а нашли "+СтруктураПроверяемыхРеквизитов.ИНАГРО_ТипРесурсов);
			Иначе
				Если СтруктураПроверяемыхРеквизитов.ВидРесурса <> Перечисления.СМ_ВидыРесурсов.МашиныИМеханизмы 
					...
					СтруктураПараметров.Вставить("ОписаниеОшибки", "Ожидали найти машинный ресурс, а нашли "+СтруктураПроверяемыхРеквизитов.ВидРесурса);
				КонецЕсли;
			КонецЕсли;
...
Функция ЗаполнитьРесурс_ПоискПоШифру(СтруктураПараметров, КэшРесурсов)
	...
	Для каждого СтрокаМассива Из МассивСтрок Цикл			
		...
		Если СтруктураПараметров.ЭтоЗарплатаМашинистов
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Ресурс с кодом "+СтруктураПараметров.Шифр + " в смете значится как машинисты, а в базе 1с ресурс другого типа - нужно проверить, возможны искажения стоимостей");
		КонецЕсли;
...
Процедура ЗагрузитьФайл_Завершение(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт 
	...
	Если РаботыДЗ.ПолучитьЭлементы().Количество() > 0 и ПоказательСП =0 Тогда
...
//2.4.2.4
// добавлен обход ошибки когад в файл по отриц ресурсам записаны положительные суммы затрат (а должны быть с минусом)
//2.4.2.5
