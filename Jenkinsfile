        ...
        stage("Обновление тестового контура") {
            steps {
               timestamps  {
                   ...
                   cmd("deployka dbupdate ${connectionstring} -storage-user ${env.storage_Usr} -storage-pwd ${env.Storage_Psw} -db-user ${env.storage_Usr} -db-pwd ${env.Storage_Psw} -allow-warnings -uccode \"123\"")
                    }
            }
        }
    }
