#language: ru
@tree

#report.epic=Документы
#report.feature=УГПР
#report.story=Применение коэф при создании работы из НС

Функционал: Как пользователь УСП я хочу иметь возможность 
  применять коэф при создании работы из НС

Сценарий: Я выполняю проверку применение коэф при создании работы из НС

Дано Я запускаю сценарий открытия TestClient или подключаю уже существующий

Я открываю тестовый документ УГПР    
  Дано Я открываю основную форму списка документа "ИНАГРО_УГПР"
  Тогда открылось окно 'Управленческий график производства работ'
  И из выпадающего списка с именем "КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Значение" я выбираю по строке '#3940 Применение коэф при создании работы из НС'
  И Я убираю группировки в текущем списке
  И в таблице "Список" я перехожу к строке:
  | 'Проект'|
  |'#3940 Применение коэф при создании работы из НС'|   
  И в таблице "Список" я выбираю текущую строку
  Тогда открылось окно 'Управленческий график производства работ *'    	    
  И я перехожу к закладке "Дерево работ"

Я проверяю применение коэфициента к стоимости материалов
  И в таблице "РаботыДЗ" я перехожу к строке:
  | 'WBS'                                                 | 'Код WBS' |
  | 'Монтаж лифта пассажирского, грузоподъемностью 630кг' | '1'       |	
  И в таблице "РаботыДЗ" поле с именем "РаботыДЗСтоимостьПрямыхЗатрат" имеет значение "8 000,00"
  И я перехожу к закладке "Материалы"
  И пауза 1
  И в таблице "РесурсыМатериалы" поле с именем "РесурсыМатериалыСумма" имеет значение "4 000,00"
  И я перехожу к закладке "Коэффициенты"
  И в таблице "КоэффициентыПересчета" я нажимаю на кнопку с именем 'КоэффициентыПересчетаДобавить'
  Тогда открылось окно 'Список поправок по техническим частям'
  И в таблице "Список" я перехожу к строке:
  | 'Обоснование'             | 'СМ'     | 
  | 'Демонтаж тест = Мат 0,9' | '0,9000' |
  И я нажимаю на кнопку с именем 'ФормаВыбрать'
  Тогда открылось окно 'Управленческий график производства работ *'
  И в таблице "КоэффициентыПересчета" поле с именем "КоэффициентыПересчетаПоправка" имеет значение "Демонтаж тест = Мат 0,9"
  И в таблице "КоэффициентыПересчета" поле с именем "КоэффициентыПересчетаКоэффициентКСтоимостиМатериалов" имеет значение "0,9000"
  И в таблице "РаботыДЗ" поле с именем "РаботыДЗСтоимостьПрямыхЗатрат" имеет значение "7 600,00"
  И я перехожу к закладке "Материалы"
  И пауза 1
  И в таблице "РесурсыМатериалы" поле с именем "РесурсыМатериалыСумма" имеет значение "3 600,00"

Я проверяю применение коэфициента к стоимости труда
  И в таблице "РаботыДЗ" я перехожу к строке:
  | 'WBS'                                                  | 'Код WBS' |
  | 'Монтаж лифта пассажирского, грузоподъемностью 1000кг' | '2'       |	
  И в таблице "РаботыДЗ" поле с именем "РаботыДЗСтоимостьПрямыхЗатрат" имеет значение "8 000,00"
  И я перехожу к закладке "Трудовые ресурсы"
  И пауза 1
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеСумма" имеет значение "4 000,00"
  И я перехожу к закладке "Коэффициенты"
  И в таблице "КоэффициентыПересчета" я нажимаю на кнопку с именем 'КоэффициентыПересчетаДобавить'
  Тогда открылось окно 'Список поправок по техническим частям'
  И в таблице "Список" я перехожу к строке:
  | 'Обоснование'             | 'ОЗ'     | 
  | 'Демонтаж тест = ОЗ= 0,8' | '0,8000' | 
  И я нажимаю на кнопку с именем 'ФормаВыбрать'
  Тогда открылось окно 'Управленческий график производства работ *'
  И в таблице "КоэффициентыПересчета" поле с именем "КоэффициентыПересчетаПоправка" имеет значение "Демонтаж тест = ОЗ= 0,8"
  И в таблице "КоэффициентыПересчета" поле с именем "КоэффициентыПересчетаКоэффициентКОсновнойЗарплате" имеет значение "0,8000"
  И в таблице "РаботыДЗ" поле с именем "РаботыДЗСтоимостьПрямыхЗатрат" имеет значение "7 200,00"
  И я перехожу к закладке "Трудовые ресурсы"
  И пауза 1
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеСумма" имеет значение "3 200,00"


Я проверяю применение коэфициента к норме труда
  И в таблице "РаботыДЗ" я перехожу к строке:
  | 'WBS'                                                  | 'Код WBS' |
  | 'Монтаж стоек забора'                                  | '3'       |	
  И в таблице "РаботыДЗ" поле с именем "РаботыДЗСтоимостьПрямыхЗатрат" имеет значение "4 231,00"
  И я перехожу к закладке "Трудовые ресурсы"
  И пауза 1
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеСумма" имеет значение "4 000,00"
  И я перехожу к закладке "Коэффициенты"
  И в таблице "КоэффициентыПересчета" я нажимаю на кнопку с именем 'КоэффициентыПересчетаДобавить'
  Тогда открылось окно 'Список поправок по техническим частям'
  И в таблице "Список" я перехожу к строке:
  | 'Обоснование'             | 'ЗТР'     | 
  | 'Демонтаж тест = ТР 0,8'  | '0,8000' | 
  И я нажимаю на кнопку с именем 'ФормаВыбрать'
  Тогда открылось окно 'Управленческий график производства работ *'
  И в таблице "КоэффициентыПересчета" поле с именем "КоэффициентыПересчетаПоправка" имеет значение "Демонтаж тест = ТР 0,8"
  И в таблице "КоэффициентыПересчета" поле с именем "КоэффициентыПересчетаКоэффициентКТрудозатратамРабочих" имеет значение "0,8000"
  И в таблице "РаботыДЗ" поле с именем "РаботыДЗСтоимостьПрямыхЗатрат" имеет значение "3 431,00"
  И я перехожу к закладке "Трудовые ресурсы"
  И пауза 1
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеСумма" имеет значение "3 200,00"
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеНормаРасхода" имеет значение "0,800000"
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеКоличество" имеет значение "16,000000"


Я проверяю применение коэфициента к МиМ
  И в таблице "РаботыДЗ" я перехожу к строке:
  | 'WBS'                                            | 'Код WBS' |
  | 'Работа на отвале бульдозерами Shantui SD-16'    | '4'       |	
  И в таблице "РаботыДЗ" поле с именем "РаботыДЗСтоимостьПрямыхЗатрат" имеет значение "27,72"
  И я перехожу к закладке "Машинные ресурсы"
  И пауза 1
  И в таблице "РесурсыМашинные" поле с именем "РесурсыМашинныеСумма" имеет значение "14,44"
  И я перехожу к закладке "Коэффициенты"
  И в таблице "КоэффициентыПересчета" я нажимаю на кнопку с именем 'КоэффициентыПересчетаДобавить'
  Тогда открылось окно 'Список поправок по техническим частям'
  И в таблице "Список" я перехожу к строке:
  | 'Обоснование'             | 'ЭМ'     | 
  | 'Машины тест = ЭМ  0,75'  | '0,7500' | 
  И я нажимаю на кнопку с именем 'ФормаВыбрать'
  Тогда открылось окно 'Управленческий график производства работ *'
  И в таблице "КоэффициентыПересчета" поле с именем "КоэффициентыПересчетаПоправка" имеет значение "Машины тест = ЭМ  0,75"
  И в таблице "КоэффициентыПересчета" поле с именем "КоэффициентыПересчетаКоэффициентКЭксплуатацииМашин" имеет значение "0,7500"
  И в таблице "РаботыДЗ" поле с именем "РаботыДЗСтоимостьПрямыхЗатрат" имеет значение "24,11"
  И я перехожу к закладке "Машинные ресурсы"
  И пауза 1
  И в таблице "РесурсыМашинные" поле с именем "РесурсыМашинныеСумма" имеет значение "10,83"

Я проверяю применение коэфициента к норме машинистов
  И в таблице "РаботыДЗ" я перехожу к строке:
  | 'WBS'                                            | 'Код WBS' |
  | 'Работа на отвале бульдозерами Shantui SD-16'    | '5'       |	
  И в таблице "РаботыДЗ" поле с именем "РаботыДЗСтоимостьПрямыхЗатрат" имеет значение "27,72"
  И я перехожу к закладке "Трудовые ресурсы"
  И пауза 1
  И в таблице "РесурсыТрудовые" я перехожу к строке:
  | 'Вид ресурса'              | 'Ресурс'                   | 'Сумма' |
  | 'Затраты труда машинистов' | 'Затраты труда машинистов' | '14,84' |
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеСумма" имеет значение "14,84"
  И я перехожу к закладке "Коэффициенты"
  И в таблице "КоэффициентыПересчета" я нажимаю на кнопку с именем 'КоэффициентыПересчетаДобавить'
  Тогда открылось окно 'Список поправок по техническим частям'
  И в таблице "Список" я перехожу к строке:
  | 'Обоснование'             | 'ЗТМ'    | 
  | 'Машины тест = ТМ  0,5'  | '0,5000' | 
  И я нажимаю на кнопку с именем 'ФормаВыбрать'
  Тогда открылось окно 'Управленческий график производства работ *'
  И в таблице "КоэффициентыПересчета" поле с именем "КоэффициентыПересчетаПоправка" имеет значение "Машины тест = ТМ  0,5"
  И в таблице "КоэффициентыПересчета" поле с именем "КоэффициентыПересчетаКоэффициентКТрудозатратамМашинистов" имеет значение "0,5000"
  И в таблице "РаботыДЗ" поле с именем "РаботыДЗСтоимостьПрямыхЗатрат" имеет значение "27,72"
  И я перехожу к закладке "Трудовые ресурсы"
  И пауза 1
  И в таблице "РесурсыТрудовые" я перехожу к строке:
  | 'Вид ресурса'              | 'Ресурс'                   |
  | 'Затраты труда машинистов' | 'Затраты труда машинистов' |
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеСумма" имеет значение "7,42"
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеНормаРасхода" имеет значение "0,001855"
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеКоличество" имеет значение "0,037100"  
  

Я проверяю применение коэфициента к оплате машинистов
  И в таблице "РаботыДЗ" я перехожу к строке:
  | 'WBS'                                            | 'Код WBS' |
  | 'Работа на отвале бульдозерами Shantui SD-16'    | '6'       |	
  И в таблице "РаботыДЗ" поле с именем "РаботыДЗСтоимостьПрямыхЗатрат" имеет значение "27,72"
  И я перехожу к закладке "Трудовые ресурсы"
  И пауза 1
  И в таблице "РесурсыТрудовые" я перехожу к строке:
  | 'Вид ресурса'              | 'Ресурс'                   | 'Сумма' |
  | 'Затраты труда машинистов' | 'Затраты труда машинистов' | '14,84' |
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеСумма" имеет значение "14,84"
  И я перехожу к закладке "Коэффициенты"
  И в таблице "КоэффициентыПересчета" я нажимаю на кнопку с именем 'КоэффициентыПересчетаДобавить'
  Тогда открылось окно 'Список поправок по техническим частям'
  И в таблице "Список" я перехожу к строке:
  | 'Обоснование'             | 'ЗМ'    | 
  | 'Машины тест = ЗПМ  0,85'  | '0,8500' | 
  И я нажимаю на кнопку с именем 'ФормаВыбрать'
  Тогда открылось окно 'Управленческий график производства работ *'
  И в таблице "КоэффициентыПересчета" поле с именем "КоэффициентыПересчетаПоправка" имеет значение "Машины тест = ЗПМ  0,85"
  И в таблице "КоэффициентыПересчета" поле с именем "КоэффициентыПересчетаКоэффициентКЗарплатеМашинистов" имеет значение "0,8500"
  И в таблице "РаботыДЗ" поле с именем "РаботыДЗСтоимостьПрямыхЗатрат" имеет значение "27,72"
  И я перехожу к закладке "Трудовые ресурсы"
  И пауза 1
  И в таблице "РесурсыТрудовые" я перехожу к строке:
  | 'Вид ресурса'              | 'Ресурс'                   |
  | 'Затраты труда машинистов' | 'Затраты труда машинистов' |
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеСумма" имеет значение "12,61"
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеНормаРасхода" имеет значение "0,003710"
  И в таблице "РесурсыТрудовые" поле с именем "РесурсыТрудовыеКоличество" имеет значение "0,074200"  
  
Я выбираю работу монтажа и делаю из нее демонтаж как существующую работу
  И в таблице "РаботыДЗ" я перехожу к строке по шаблону:
  | 'WBS'                                                                            | 'Код WBS' |
  | '* металлоконструкции лифта пассажирского, грузоподъемностью 630-1000кг'    | '7'       |	
  И пауза 1
  И я перехожу к закладке "Материалы"
  И пауза 1
  Если в таблице "РесурсыМатериалы" количество строк "равно" 0 Тогда
    И в таблице "РесурсыМатериалы" я нажимаю на кнопку с именем 'РесурсыМатериалыДобавить'
    И в таблице "РесурсыМатериалы" из выпадающего списка с именем "РесурсыМатериалыРесурс" я выбираю по строке 'Материал1'
    И в таблице "РесурсыМатериалы" я завершаю редактирование строки
  И в таблице "РаботыДЗ" я активизирую поле "WBS"
  И в таблице "РаботыДЗ" я выбираю текущую строку  
  И я нажимаю на кнопку открытия поля с именем "РаботыДЗВидРаботы"
  Тогда открылось окно '*Виды работ (проект)'  
  И я устанавливаю флаг 'Изменено название'
  И я снимаю флаг 'Демонтаж'
  И я устанавливаю флаг 'Демонтаж'
  Тогда элемент формы с именем "Наименование" стал равен 'Демонтаж металлоконструкции лифта пассажирского, грузоподъемностью 630-1000кг'
  И я нажимаю на кнопку 'Записать и закрыть'  
  Тогда открылось окно 'Управленческий график производства работ *'
  И в таблице "РаботыДЗ" я завершаю редактирование строки
  И я перехожу к закладке "Материалы"
  # в демонтаже материалы удаляються
  И я жду, что в таблице "РесурсыМатериалы" количество строк будет "равно" 0 в течение 10 секунд

Я выбираю работу монтажа и делаю из нее демонтаж как новую работу 
  И в таблице "РаботыДЗ" я нажимаю на кнопку 'Работу(ы)'
  Тогда открылось окно 'Виды работ (проект)'
  И в таблице "Список" я перехожу к строке:
  | 'Наименование'       |
  | 'Монтаж лифта пассажирского, грузоподъемностью 1000кг' |
 



