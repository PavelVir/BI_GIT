...
И В командном интерфейсе я выбираю 'Управление строительным производством' 'Мастер сопоставления работ (Доход и расход)'
...
Делаем копию документа с версий приемником
    ...
    И в таблице "Список" я выбираю текущую строку
    Тогда открылось окно 'Бюджет проекта переменных затрат * Перенос соответствий вручную'
    И я перехожу к закладке "Основное"
    ...
    и пауза 1
    И я активизирую окно "Бюджет проекта переменных затрат"
    И в таблице "Список" я выбираю текущую строку
...
Допустим В командном интерфейсе я выбираю 'Управление строительным производством' 'Мастер сопоставления работ (Доход и расход)'
Тогда открылось окно 'Мастер сопоставления работ: Форма мастера'
И из выпадающего списка "Проект" я выбираю по строке 'Сценарный тест' 
И элемент формы с именем "Версия" стал равен  шаблону "(*) 2830 Перенос соответствий внешняя"
И из выпадающего списка "Рабочая версия" я выбираю по строке '2830 Перенос соответствий вручную'
