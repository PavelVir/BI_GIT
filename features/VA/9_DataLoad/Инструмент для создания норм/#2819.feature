...
Сценарий: Я демонстрирую использование инструмента создания норм

#[autodoc.groupsteps] Я открываю Инструмент создания норм и заполняю первоначальный настройки
...
#[autodoc.groupsteps]  Я ввожу наименование шаблона, которое включает макеты свойств в квадратных скобках
...
#[autodoc.groupsteps]  Проверяем как запомнились настройки формирования соответствий свойств и ограничений из регистра по текущему шаблону
...
Проверяем формирование  материального состава работ
	...
	#[autodoc.groupsteps]  Проверяю что он есть в работе
	...
	#[autodoc.groupsteps] Повторная запись для проверки что выводится сообщение
	Повторная запись для проверки что выводится сообщение	
		И в таблице "РесурсыМатериальныйСостав" я нажимаю на кнопку 'Записать материалы'
		Если появилось окно с заголовком 'В указанных видах работ уже заполнен *' Тогда
			И я нажимаю на кнопку 'Да'
		И в таблице "СоставРаботВыбранныеВидыРабот" я выбираю текущую строку
...
Проверяем формирование состава звена
	#[autodoc.groupsteps] Заполняем машины и механизмы
	...
	#[autodoc.groupsteps] Проверим заполнение вида работ
	...
	#[autodoc.groupsteps] Проверим перезапись чтобы не было дублирования
