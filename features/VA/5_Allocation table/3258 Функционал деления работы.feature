...
Я делаю распределительную ведомость с частью работ на субподряд
    ...
    Я делю работы на исполнителей   
        ...
        Третью работу делаим на три части, первую часть подрядику с ресурсами, вторую другому подразделению
            И в таблице "РаботыДЗ" я перехожу к строке:
            | 'Вид работы'  | 'Ед.Изм'  | 'Заявка на тендер' | 'Объем'     | 'Подразделение' | 'Формировать документы условий' |
...
Я проверяю результат деления в УГПР
    ...
    Тогда таблица "РаботыДЗ" стала равной:
    | 'WBS'                                                                  | 'Код WBS' | 'ЕдИзм'   | 'Длительность, дн' | 'Себест. единицы' | 'Шифр позиции норматива' | 'Стоимость прямых затрат' | 'Себестоимость всего' | 'Стоимость заказчика' | 'НДС' | 'Маржа'     | 'Субподряд' |
    | 'Итоги по работам'                                                     | ''        | ''        | '20'               | ''                | ''                       | '7 437,78'                | '7 437,78'            | '7 500,01'            | ''    | '62,23'     | 'Нет'       |
    | 'Земляные работы'                                                      | '0'       | ''        | '20'               | ''                | ''                       | '7 437,78'                | '7 437,78'            | '7 500,01'            | ''    | '62,23'     | 'Нет'       |
    | 'Валка деревьев мягких пород с корня при диаметре стволов более 32 см' | '1'       | '1дерево' | '6'                | '255,08'          | 'Е1-191-6'               | '2 550,79'                | '2 550,79'            | '2572,13'             | ''    | '21,34'     | 'Нет'       |
    | 'Валка деревьев мягких пород с корня при диаметре стволов более 32 см' | '1'       | '1дерево' | '6'                | '255,08'          | 'Е1-191-6'               | '2 550,79'                | '2 550,79'            | '2572,13'             | ''    | '21,34'     | 'Да'        |
    | 'Засыпка подкоренных ям бульдозерами мощностью 118 кВт (160 л.с.)'     | '2'       | 'яма'     | '1'                | '6,31'            | 'Е1-199-2'               | '63,10'                   | '63,10'               | '63,62'               | ''    | '0,52'      | 'Да'        |
    | 'Засыпка подкоренных ям бульдозерами мощностью 118 кВт (160 л.с.)'     | '2'       | 'яма'     | '1'                | '6,31'            | 'Е1-199-2'               | '63,10'                   | '63,10'               | '63,63'               | ''    | '0,53'      | 'Нет'       |
    | 'Водоотлив из котлованов'                                              | '3'       | 'м3'      | '2'                | '442,00'          | 'Е1-173-2'               | '884,00'                  | '884,00'              | '891,40'              | ''    | '7,39'      | 'Нет'       |
    | 'Водоотлив из котлованов'                                              | '3'       | 'м3'      | '2'                | '442,00'          | 'Е1-173-2'               | '884,00'                  | '884,00'              | '891,40'              | ''    | '7,40'      | 'Нет'       |
    | 'Водоотлив из котлованов'                                              | '3'       | 'м3'      | '2'                | '442,00'          | 'Е1-173-2'               | '442,00'                  | '442,00'              | '445,70'              | ''    | '3,70'      | 'Да'        |
